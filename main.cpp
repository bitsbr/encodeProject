...
static void splitFiles(const char *filePath, int numberParts)
{
    ...
    for (int i = pathInfo.str().size(); i > 0; i--)
    {
        if (filePath[i] == '.')
        {
            std::cout << "Not founded in part of string : " << i << std::endl;
...
static void joinFiles()
{
    ...
    char Buffer[size];
    int BufferPartsNumber;

    iFile >> BufferPartsNumber;
    ...
    std::cout << "Original Parts Value : " << BufferPartsNumber << std::endl;
    std::cout << "File Data : " << Buffer << std::endl;

    std::ostringstream teste;
    teste << Buffer;

    std::cout << teste.str() << std::endl;

    for (int i = 0; i < BufferPartsNumber; i++)
    {
        std::cout << "Writing part " << i << std::endl;
    }
}
